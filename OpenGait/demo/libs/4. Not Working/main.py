...
def main():
    ...
    for gallery_video_path in gallery_video_paths:
        ...
        if os.path.exists(gallery_feature_path):
            ...
            features = extract_sil(gallery_silhouettes[gallery_video_path], feature_folder, subject_id, view_id, seq_id)
            save_features(features, gallery_feature_path)
        
        # print("    >> Checking extracted feature keys:", features.keys())
        gallery_features.update(features)  # <-- THIS is essential
        print("    >> Updated gallery_features keys:", gallery_features.keys())
    ...
    for probe_video_path in probe_video_paths:
        ...
        if os.path.exists(probe_feature_path):
            ...
            probe_features = extract_sil(probe_silhouette, feature_folder, subject_id, view_id, seq_id)
