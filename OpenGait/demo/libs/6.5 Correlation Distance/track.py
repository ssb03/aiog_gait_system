...
def writeresult(pgdict, video_path, video_save_folder):
    ...
    if pgdict[first_key] is None:
        gallery_name = "no_match"
    else:
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                for t in online_targets:
                    ...
                    tid = pgdict.get(pid, "no_match")

                    if tid == "no_match":
                        colorid = 999  # Assign a default color id
                    else:
                        try:
                            colorid = int(tid.split("-")[1])
                        except (IndexError, ValueError):
                            print(f"[Warning] Failed to parse color id from tid: '{tid}'")
                            colorid = 999
